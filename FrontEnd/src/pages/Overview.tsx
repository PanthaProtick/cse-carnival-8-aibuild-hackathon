...
export function Overview(){
  const results=useQueries({queries:resources.map(resource=>({queryKey:[resource],queryFn:()=>api.list(resource)}))}); const me=useQuery({queryKey:['me'],queryFn:api.me})
  ...
  const [schedules,rooms,events,announcements,assignments]=results.map(item=>item.data!); const pending=assignments.items.filter((item:any)=>item.status==='pending'); const nextEvents=events.items.filter((item:any)=>item.status==='upcoming').slice(0,3)
